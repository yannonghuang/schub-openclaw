...
#
# OpenClaw's chat-completions routing uses the sessionKey (e.g. agent:scheduling:
...
set -e
DIR=/home/node/.openclaw/agents/scheduling/sessions
...
hits = [(k, v) for k, v in data.items()
        if v.get("sessionId") == "$SID" and k.startswith("agent:scheduling:subagent:")]
