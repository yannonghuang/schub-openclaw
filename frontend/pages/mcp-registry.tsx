...
export default function MCPRegistry() {
  ...
  const BusinessEditor = ({ business, onSave }) => {
    const [form, setForm] = useState(business);
    return (
      <div className="space-y-2 mb-8">
        <h2 className="text-xl font-bold mb-2">Main Agent Prompt</h2>
        <textarea
          value={form.agent_prompt ?? ""}
          rows={5}
          cols={40}
          onChange={e => setForm({ ...form, agent_prompt: e.target.value })}
  ...
  return (
    <div className="p-4 space-y-10">
      ...
      {/* ===================== MAIN BUSINESS ===================== */}
      <BusinessEditor business={business} onSave={f => saveBusiness(business.id, f)} />

      {/* ===================== SUB-AGENTS ===================== */}
      <div>
        <h2 className="text-xl font-bold mb-3">Sub-agents</h2>
        ...
        <ul className="space-y-2">
          {subAgents.map(sa => (
            <li key={sa.id} className="border p-2 rounded">
              {/* Editing sub-agent */}
              {editingSubAgent?.id === sa.id ? (
                <SubAgentEditor
                  sa={sa}
                  onSave={form => saveSubAgent(sa.id, form)}
                  onCancel={() => setEditingSubAgent(null)}
                />
              ) : (
                <div className="flex justify-between">
                  <span
                    className="cursor-pointer underline"
                    onClick={() => loadSubAgentDetails(sa)}
                  >
                    {sa.name}
                  </span>
      ...
      {selectedSubAgent && (
        <div className="border p-4 rounded bg-gray-50">
          <h2 className="text-xl font-bold mb-3">
            Sub-agent: {selectedSubAgent.name}
          </h2>
